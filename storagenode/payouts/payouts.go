...
type DB interface {
	...
	GetPayStub(ctx context.Context, satelliteID storxnetwork.NodeID, period string) (*PayStub, error)
	...
	SatellitesHeldbackHistory(ctx context.Context, satelliteID storxnetwork.NodeID) ([]HeldForPeriod, error)
	// SatellitesDisposedHistory returns all disposed amount for specific satellite from DB.
	SatellitesDisposedHistory(ctx context.Context, satelliteID storxnetwork.NodeID) (int64, error)
	// SatellitePeriods retrieves all periods for concrete satellite in which we have some payouts data.
	SatellitePeriods(ctx context.Context, satelliteID storxnetwork.NodeID) ([]string, error)
	...
	GetReceipt(ctx context.Context, satelliteID storxnetwork.NodeID, period string) (string, error)
	...
	GetEarnedAtSatellite(ctx context.Context, id storxnetwork.NodeID) (int64, error)
	// GetPayingSatellitesIDs returns list of satellite ID's that ever paid to storagenode.
	GetPayingSatellitesIDs(ctx context.Context) ([]storxnetwork.NodeID, error)
	// GetSatelliteSummary returns satellite all time paid and held amounts.
	GetSatelliteSummary(ctx context.Context, satelliteID storxnetwork.NodeID) (paid, held int64, err error)
	// GetSatellitePeriodSummary returns satellite paid and held amounts for specific period.
	GetSatellitePeriodSummary(ctx context.Context, satelliteID storxnetwork.NodeID, period string) (paid, held int64, err error)
	...
	GetSatellitePaystubs(ctx context.Context, satelliteID storxnetwork.NodeID) (*PayStub, error)
	...
	GetSatellitePeriodPaystubs(ctx context.Context, period string, satelliteID storxnetwork.NodeID) (*PayStub, error)
