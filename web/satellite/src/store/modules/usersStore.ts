...
export const useUsersStore = defineStore("users", () => {
  ...
  const shouldOnboard = computed(() => {
    return (
      !state.settings.onboardingStart ||
      (state.settings.onboardingStart && !state.settings.onboardingEnd)
    );
  });
  ...
  function getShouldPromptPassphrase(data: {
    isProjectOwner: boolean;
    onboardingStepperEnabled: boolean;
  }): boolean {
    const settings = state.settings;
    const step =
      (settings.onboardingStep as OnboardingStep) ||
    ...
    if (data.isProjectOwner && data.onboardingStepperEnabled) {
      return (
        settings.onboardingEnd || step !== OnboardingStep.EncryptionPassphrase
      );
    }
    return true;
  }
  ...
  async function regenerateUserMFARecoveryCodes(code: {
    recoveryCode?: string;
    passcode?: string;
  }): Promise<void> {
    const codes = await api.regenerateUserMFARecoveryCodes(
      code.passcode,
      code.recoveryCode,
    );
