...
<template>
    <v-container>
        ...
        <v-alert v-else-if="error" type="error" variant="tonal" class="ma-4">
            <v-alert-title>Error loading account details</v-alert-title>
            {{ error }}
        </v-alert>
                ...
                <v-card title="Account" :subtitle="userAccount.fullName" variant="flat" :border="true" rounded="xlg">
                    <v-card-text>
                        ...
                        <div class="mt-2 text-caption text-medium-emphasis">
                            Project Limit: {{ userAccount.projects?.length || 0 }} / {{ userAccount.projectLimit || 'Unlimited' }}
                        </div>
        ...
        <v-row>
            ...
            <v-col cols="12" sm="6" md="3">
                <card-stats-component title="Storage" subtitle="Total" :data="sizeToBase10String(totalUsage.storage)" />
            </v-col>
            ...
            <v-col cols="12" sm="6" md="3">
                <card-stats-component title="Segments" subtitle="Total" :data="totalUsage.segments.toLocaleString()" />
            </v-col>
        </v-row>
...
const totalUsage = computed<Usage>(() => {
    ...
    for (const project of userAccount.value.projects) {
        // Sum storage usage (treat null/undefined as 0)
        const storageUsed = project.storageUsed || 0;
        total.storage += storageUsed;

        // Sum bandwidth usage (treat null/undefined as 0)
        const bandwidthUsed = project.bandwidthUsed || 0;
        total.download += bandwidthUsed;

        // Sum segment usage (treat null/undefined as 0)
        const segmentUsed = project.segmentUsed || 0;
        total.segments += segmentUsed;
    }
...
const usageCacheError = computed<boolean>(() => {
    return !!userAccount.value.projects?.some(project =>
        ...
        project.storageUsed === undefined ||
        project.segmentUsed === null ||
        project.segmentUsed === undefined,
    );
});
...
const loadUserData = async () => {
    // Get email from URL parameters first, then localStorage as fallback
    const route = useRoute();
    const emailFromUrl = route.query.email as string;
    const emailFromStorage = localStorage.getItem('selectedUserEmail');
    ...
    try {
        ...
        // Clear existing user data before loading new data
        appStore.state.userAccount = null;
        
        const userInfo = await adminApi.getUserInfo(email);
        
        // Map the API response to UserAccount format
        const userAccount: UserAccount = {
            id: userInfo.user.id,
            fullName: userInfo.user.fullName,
            email: userInfo.user.email,
            paidTier: userInfo.user.paidTier,
            createdAt: userInfo.user.createdAt,
            status: userInfo.user.status === 1 ? 'Active' : 'Inactive',
            userAgent: userInfo.user.userAgent || 'Unknown',
            defaultPlacement: userInfo.user.placement,
            projectLimit: userInfo.user.projectLimit,
            projects: userInfo.projects ? userInfo.projects.map(project => ({
                id: project.id,
                name: project.name,
                bandwidthLimit: project.bandwidthLimit || 0,
                bandwidthUsed: project.bandwidthUsed || 0,
                storageLimit: project.storageLimit || 0,
                storageUsed: project.storageUsed || 0,
                segmentLimit: project.segmentLimit || 0,
                segmentUsed: project.segmentUsed || 0,
            })) : []
        };
        
        appStore.state.userAccount = userAccount;
    } catch (err) {
...
onBeforeMount(() => {
    // Always load user data when component mounts
    loadUserData();
});
