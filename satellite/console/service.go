...
func (payment Payments) AddCreditCard(ctx context.Context, creditCardToken string) (card payments.CreditCard, err error) {
	...
	payment.service.SendNotificationAsync(user.ID, user.Email, "credit_card_added", "billing", variables)
...
func (payment Payments) RemoveCreditCard(ctx context.Context, cardID string) (err error) {
	...
	payment.service.SendNotificationAsync(user.ID, user.Email, "credit_card_removed", "billing", variables)
...
func (s *Service) DeleteProject(ctx context.Context, projectID uuid.UUID) (err error) {
	...
	s.SendNotificationAsync(user.ID, user.Email, "project_deleted", "account", variables)
...
func (s *Service) RequestLimitIncrease(ctx context.Context, projectID uuid.UUID, info LimitRequestInfo) (err error) {
	...
	s.SendNotificationAsync(user.ID, user.Email, "limit_increase_requested", "account", variables)
...
func (s *Service) AddProjectMembers(ctx context.Context, projectID uuid.UUID, emails []string) (users []*User, err error) {
	...
	for _, addedUser := range users {
		variables := map[string]interface{}{
			"project_name": projectName,
			"member_email": addedUser.Email,
		}
		s.SendNotificationAsync(addedUser.ID, addedUser.Email, "project_member_added", "account", variables)
	}
	...
	go func() {
		...
		for _, email := range emails {
			...
			if err == nil {
				variables := map[string]interface{}{
					"project_name": projectName,
					"member_email": deletedUser.Email,
				}
				s.SendNotificationAsync(deletedUser.ID, deletedUser.Email, "project_member_deleted", "account", variables)
			}
		}
	}()
...
// Dashboard card types for GetDashboardStats
...
// GetDashboardStats aggregates dashboard cards data for a user.
// tokenGetter is a function that returns the authentication token for external API calls.
...
func (s *Service) enrichBillingCard(ctx context.Context, card *BillingCard, user *User) {
	...
	card.Status = "active"
	transactions, err := s.GetBillingTransactions(ctx, user.ID)
...
func (s *Service) enrichAccessCard(ctx context.Context, card *AccessCard, projectID uuid.UUID) {
	...
	if members, err := s.GetProjectMembersAndInvitations(ctx, projectID, ProjectMembersCursor{
		Limit: 1, Page: 1,
	}); err == nil {
