...
func (db *ProjectAccounting) GetBucketTotalsForReservedBuckets(ctx context.Context, projectID uuid.UUID) (_ []accounting.BucketUsage, err error) {
	...
	WHERE project_id = ? AND name IN ('cyberls-gmail', 'gmail', 'cyberls-drive', 'google-drive', 'google-cloud', 'google-photos', 'cyberls-calendar', 'google-calendar', 'cyberls-contacts', 'google-contacts', 'dropbox', 'aws-s3', 'github', 'shopify', 'quickbooks')
	ORDER BY name ASC`)
