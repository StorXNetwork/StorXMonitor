...
func (db *satelliteDB) productionMigrationSpanner() *migrate.Migration {
	return &migrate.Migration{
		...
		Steps: []*migrate.Step{
			...
			{
				DB:          &db.migrationDB,
				Description: "add mail_export_jobs table",
				Version:     133,
				Action: migrate.SQL{
			...
			{
				DB:          &db.migrationDB,
				Description: "mail_export_jobs download charge idempotency columns",
				Version:     134,
				Action: migrate.SQL{
					`ALTER TABLE mail_export_jobs ADD COLUMN last_download_charge_id STRING(MAX)`,
			...
			{
				DB:          &db.migrationDB,
				Description: "add seller billing plans, assignments, invoices, and notifications",
				Version:     138,
				Action: migrate.SQL{
					...
					`CREATE INDEX seller_plans_active_sort_index ON seller_plans ( active, sort_order )`,
			...
			{
				DB:          &db.migrationDB,
				Description: "add description, features (json), recommended to seller_plans",
				Version:     139,
				Action: migrate.SQL{
					`ALTER TABLE seller_plans ADD COLUMN IF NOT EXISTS description STRING(MAX) NOT NULL DEFAULT ('');`,
					`ALTER TABLE seller_plans ADD COLUMN IF NOT EXISTS features JSON NOT NULL DEFAULT (JSON '[]');`,
					`ALTER TABLE seller_plans ADD COLUMN IF NOT EXISTS recommended BOOL NOT NULL DEFAULT (false);`,
				},
			},
			{
				DB:          &db.migrationDB,
				Description: "link seller_plans to payment_plans",
				Version:     140,
				Action: migrate.SQL{
					`ALTER TABLE seller_plans ADD COLUMN IF NOT EXISTS payment_plan_id INT64;`,
				},
			},
			{
				DB:          &db.migrationDB,
				Description: "drop unused seller_plans.sort_order",
				Version:     141,
				Action: migrate.SQL{
					`DROP INDEX IF EXISTS seller_plans_active_sort_index`,
					`ALTER TABLE seller_plans DROP COLUMN IF EXISTS sort_order`,
					`CREATE INDEX seller_plans_active_name_index ON seller_plans ( active, name )`,
				},
			},
			{
				DB:          &db.migrationDB,
				Description: "drop seller assignment invoice notification tables keep plans",
				Version:     142,
				Action: migrate.SQL{
			...
			{
				DB:          &db.migrationDB,
				Description: "recreate seller assignment invoice notification tables",
				Version:     143,
				Action: migrate.SQL{
					`CREATE TABLE IF NOT EXISTS seller_user_plan_assignments (
						id BYTES(MAX) NOT NULL,
						reseller_id BYTES(MAX) NOT NULL,
						user_id BYTES(MAX) NOT NULL,
						plan_id BYTES(MAX) NOT NULL,
					...
					`CREATE INDEX seller_invoices_reseller_id_index ON seller_invoices ( reseller_id )`,
					`CREATE TABLE IF NOT EXISTS seller_invoice_lines (
						id BYTES(MAX) NOT NULL,
						invoice_id BYTES(MAX) NOT NULL,
						assignment_id BYTES(MAX) NOT NULL,
						user_id BYTES(MAX) NOT NULL,
					...
					`CREATE INDEX seller_invoice_lines_invoice_id_index ON seller_invoice_lines ( invoice_id )`,
					`CREATE TABLE IF NOT EXISTS seller_billing_notifications (
						id BYTES(MAX) NOT NULL,
						reseller_id BYTES(MAX) NOT NULL,
						user_id BYTES(MAX),
						type STRING(MAX) NOT NULL,
...
VALUES 
(decode(replace(gen_random_uuid()::text, '-', ''), 'hex'), 'push', 'session times changed', 'account', 
'{"type": "push", "level": 3, "variables": {"session_minutes": {"type": "number", "required": true}}, "body_template": "Your session timeout has been changed to {{.session_minutes}} minutes", "title_template": "Session Settings Updated", "default_variables": {"session_minutes": 30}}'::jsonb,
true, NOW(), NOW()),
(decode(replace(gen_random_uuid()::text, '-', ''), 'hex'), 'push', 'logged in successfully', 'account',
'{"type": "push", "level": 2, "variables": {"location": {"type": "string", "required": false}, "timestamp": {"type": "string", "required": true}, "ip_address": {"type": "string", "required": true}}, "body_template": "You have successfully logged in at {{.timestamp}} from {{.location}} ({{.ip_address}})", "title_template": "Login Successful", "default_variables": {"location": "Unknown Location", "timestamp": "now", "ip_address": "0.0.0.0"}}'::jsonb,
true, NOW(), NOW()),
(decode(replace(gen_random_uuid()::text, '-', ''), 'hex'), 'push', 'name changed successfully', 'account',
'{"type": "push", "level": 2, "variables": {"old_name": {"type": "string", "required": false}, "new_name": {"type": "string", "required": true}}, "body_template": "Your account name has been successfully changed{{if .old_name}} from {{.old_name}}{{end}} to {{.new_name}}", "title_template": "Name Updated", "default_variables": {"old_name": "", "new_name": "User"}}'::jsonb,
true, NOW(), NOW()),
(decode(replace(gen_random_uuid()::text, '-', ''), 'hex'), 'push', 'password changed successfully', 'account',
'{"type": "push", "level": 4, "variables": {"location": {"type": "string", "required": false}, "timestamp": {"type": "string", "required": true}, "ip_address": {"type": "string", "required": true}}, "body_template": "Your password has been successfully changed at {{.timestamp}} from {{.location}} ({{.ip_address}})", "title_template": "Password Changed", "default_variables": {"location": "Unknown Location", "timestamp": "now", "ip_address": "0.0.0.0"}}'::jsonb,
true, NOW(), NOW()),
(decode(replace(gen_random_uuid()::text, '-', ''), 'hex'), 'push', 'settings updated', 'account',
'{"type": "push", "level": 2, "variables": {"timestamp": {"type": "string", "required": true}}, "body_template": "Your account settings have been successfully updated at {{.timestamp}}", "title_template": "Settings Updated", "default_variables": {"timestamp": "now"}}'::jsonb,
true, NOW(), NOW()),
(decode(replace(gen_random_uuid()::text, '-', ''), 'hex'), 'push', 'profile updated', 'account',
'{"type": "push", "level": 2, "variables": {"timestamp": {"type": "string", "required": true}}, "body_template": "Your profile has been successfully updated at {{.timestamp}}", "title_template": "Profile Updated", "default_variables": {"timestamp": "now"}}'::jsonb,
true, NOW(), NOW()),
(decode(replace(gen_random_uuid()::text, '-', ''), 'hex'), 'push', 'mfa enabled', 'account',
'{"type": "push", "level": 3, "variables": {"mfa_type": {"type": "string", "required": false}, "timestamp": {"type": "string", "required": true}}, "body_template": "Multi-factor authentication has been enabled{{if .mfa_type}} ({{.mfa_type}}){{end}} at {{.timestamp}}", "title_template": "MFA Enabled", "default_variables": {"mfa_type": "", "timestamp": "now"}}'::jsonb,
true, NOW(), NOW()),
(decode(replace(gen_random_uuid()::text, '-', ''), 'hex'), 'push', 'mfa disabled', 'account',
'{"type": "push", "level": 4, "variables": {"timestamp": {"type": "string", "required": true}}, "body_template": "Multi-factor authentication has been successfully disabled for your account at {{.timestamp}}", "title_template": "MFA Disabled", "default_variables": {"timestamp": "now"}}'::jsonb,
true, NOW(), NOW()),
(decode(replace(gen_random_uuid()::text, '-', ''), 'hex'), 'push', 'account frozen', 'account',
'{"type": "push", "level": 4, "variables": {"reason": {"type": "string", "required": false}, "timestamp": {"type": "string", "required": true}}, "body_template": "Your account has been frozen{{if .reason}} due to: {{.reason}}{{end}} at {{.timestamp}}", "title_template": "Account Frozen", "default_variables": {"reason": "", "timestamp": "now"}}'::jsonb,
true, NOW(), NOW()),
(decode(replace(gen_random_uuid()::text, '-', ''), 'hex'), 'push', 'account unfrozen', 'account',
'{"type": "push", "level": 3, "variables": {"timestamp": {"type": "string", "required": true}}, "body_template": "Your account has been unfrozen at {{.timestamp}}", "title_template": "Account Unfrozen", "default_variables": {"timestamp": "now"}}'::jsonb,
true, NOW(), NOW()),
(decode(replace(gen_random_uuid()::text, '-', ''), 'hex'), 'push', 'account unwarned', 'account',
'{"type": "push", "level": 2, "variables": {"timestamp": {"type": "string", "required": true}}, "body_template": "The warning on your account has been removed at {{.timestamp}}", "title_template": "Account Warning Removed", "default_variables": {"timestamp": "now"}}'::jsonb,
true, NOW(), NOW()),
(decode(replace(gen_random_uuid()::text, '-', ''), 'hex'), 'push', 'account freeze warning', 'account',
'{"type": "push", "level": 4, "variables": {"days_remaining": {"type": "number", "required": false}, "reason": {"type": "string", "required": false}, "timestamp": {"type": "string", "required": true}}, "body_template": "Warning: Your account may be frozen{{if .days_remaining}} in {{.days_remaining}} days{{end}}{{if .reason}} due to: {{.reason}}{{end}} at {{.timestamp}}", "title_template": "Account Freeze Warning", "default_variables": {"days_remaining": 0, "reason": "", "timestamp": "now"}}'::jsonb,
true, NOW(), NOW()),
(decode(replace(gen_random_uuid()::text, '-', ''), 'hex'), 'push', 'user sign up', 'account',
'{"type": "push", "level": 1, "variables": {"email": {"type": "string", "required": true}, "timestamp": {"type": "string", "required": true}}, "body_template": "Welcome! Your account has been created successfully at {{.timestamp}}", "title_template": "Welcome to StorX", "default_variables": {"email": "", "timestamp": "now"}}'::jsonb,
true, NOW(), NOW()),
(decode(replace(gen_random_uuid()::text, '-', ''), 'hex'), 'push', 'registered successfully', 'account',
'{"type": "push", "level": 4, "variables": {"email": {"type": "string", "required": true}, "timestamp": {"type": "string", "required": true}}, "body_template": "Your account has been registered successfully at {{.timestamp}}", "title_template": "Registration Successful", "default_variables": {"email": "", "timestamp": "now"}}'::jsonb,
true, NOW(), NOW()),
(decode(replace(gen_random_uuid()::text, '-', ''), 'hex'), 'push', 'account activated', 'account',
'{"type": "push", "level": 4, "variables": {"email": {"type": "string", "required": true}, "timestamp": {"type": "string", "required": true}}, "body_template": "Your account has been successfully activated at {{.timestamp}}", "title_template": "Account Activated", "default_variables": {"email": "", "timestamp": "now"}}'::jsonb,
true, NOW(), NOW()),
(decode(replace(gen_random_uuid()::text, '-', ''), 'hex'), 'push', 'account deactivated', 'account',
'{"type": "push", "level": 4, "variables": {"email": {"type": "string", "required": true}, "timestamp": {"type": "string", "required": true}}, "body_template": "Your account has been deactivated at {{.timestamp}}", "title_template": "Account Deactivated", "default_variables": {"email": "", "timestamp": "now"}}'::jsonb,
true, NOW(), NOW()),
(decode(replace(gen_random_uuid()::text, '-', ''), 'hex'), 'push', 'credit card added', 'billing',
'{"type": "push", "level": 3, "variables": {"last_four": {"type": "string", "required": false}, "card_type": {"type": "string", "required": false}, "timestamp": {"type": "string", "required": true}}, "body_template": "A credit card{{if .card_type}} ({{.card_type}}){{end}}{{if .last_four}} ending in {{.last_four}}{{end}} has been added to your account at {{.timestamp}}", "title_template": "Payment Method Added", "default_variables": {"last_four": "", "card_type": "", "timestamp": "now"}}'::jsonb,
true, NOW(), NOW()),
(decode(replace(gen_random_uuid()::text, '-', ''), 'hex'), 'push', 'credit card removed', 'billing',
'{"type": "push", "level": 3, "variables": {"last_four": {"type": "string", "required": false}, "timestamp": {"type": "string", "required": true}}, "body_template": "A credit card{{if .last_four}} ending in {{.last_four}}{{end}} has been removed from your account at {{.timestamp}}", "title_template": "Payment Method Removed", "default_variables": {"last_four": "", "timestamp": "now"}}'::jsonb,
true, NOW(), NOW()),
(decode(replace(gen_random_uuid()::text, '-', ''), 'hex'), 'push', 'default credit card changed', 'billing',
'{"type": "push", "level": 2, "variables": {"timestamp": {"type": "string", "required": true}}, "body_template": "Your default credit card has been updated at {{.timestamp}}", "title_template": "Default Credit Card Changed", "default_variables": {"timestamp": "now"}}'::jsonb,
true, NOW(), NOW()),
(decode(replace(gen_random_uuid()::text, '-', ''), 'hex'), 'push', 'cron created', 'backup',
'{"type": "push", "level": 2, "variables": {"cron_name": {"type": "string", "required": false}, "schedule": {"type": "string", "required": false}, "timestamp": {"type": "string", "required": true}}, "body_template": "A new cron job{{if .cron_name}} ({{.cron_name}}){{end}}{{if .schedule}} scheduled for {{.schedule}}{{end}} has been created at {{.timestamp}}", "title_template": "Cron Job Created", "default_variables": {"cron_name": "", "schedule": "", "timestamp": "now"}}'::jsonb,
true, NOW(), NOW()),
(decode(replace(gen_random_uuid()::text, '-', ''), 'hex'), 'push', 'cron started running', 'backup',
'{"type": "push", "level": 2, "variables": {"cron_name": {"type": "string", "required": false}, "timestamp": {"type": "string", "required": true}}, "body_template": "Cron job{{if .cron_name}} {{.cron_name}}{{end}} has started running at {{.timestamp}}", "title_template": "Cron Job Started", "default_variables": {"cron_name": "", "timestamp": "now"}}'::jsonb,
true, NOW(), NOW()),
(decode(replace(gen_random_uuid()::text, '-', ''), 'hex'), 'push', 'cron successfully completed', 'backup',
'{"type": "push", "level": 2, "variables": {"cron_name": {"type": "string", "required": false}, "duration": {"type": "string", "required": false}, "timestamp": {"type": "string", "required": true}}, "body_template": "Cron job{{if .cron_name}} {{.cron_name}}{{end}} has successfully completed{{if .duration}} in {{.duration}}{{end}} at {{.timestamp}}", "title_template": "Cron Job Completed", "default_variables": {"cron_name": "", "duration": "", "timestamp": "now"}}'::jsonb,
true, NOW(), NOW()),
(decode(replace(gen_random_uuid()::text, '-', ''), 'hex'), 'push', 'cron failed', 'backup',
'{"type": "push", "level": 4, "variables": {"cron_name": {"type": "string", "required": false}, "error_message": {"type": "string", "required": false}, "timestamp": {"type": "string", "required": true}}, "body_template": "Cron job{{if .cron_name}} {{.cron_name}}{{end}} has failed{{if .error_message}}: {{.error_message}}{{end}} at {{.timestamp}}", "title_template": "Cron Job Failed", "default_variables": {"cron_name": "", "error_message": "", "timestamp": "now"}}'::jsonb,
true, NOW(), NOW()),
(decode(replace(gen_random_uuid()::text, '-', ''), 'hex'), 'push', 'scheduled task created', 'backup',
'{"type": "push", "level": 2, "variables": {"task_name": {"type": "string", "required": false}, "schedule": {"type": "string", "required": false}, "timestamp": {"type": "string", "required": true}}, "body_template": "A new scheduled task{{if .task_name}} ({{.task_name}}){{end}}{{if .schedule}} scheduled for {{.schedule}}{{end}} has been created at {{.timestamp}}", "title_template": "Scheduled Task Created", "default_variables": {"task_name": "", "schedule": "", "timestamp": "now"}}'::jsonb,
true, NOW(), NOW()),
(decode(replace(gen_random_uuid()::text, '-', ''), 'hex'), 'push', 'scheduled task started running', 'backup',
'{"type": "push", "level": 2, "variables": {"task_name": {"type": "string", "required": false}, "timestamp": {"type": "string", "required": true}}, "body_template": "Scheduled task{{if .task_name}} {{.task_name}}{{end}} has started running at {{.timestamp}}", "title_template": "Scheduled Task Started", "default_variables": {"task_name": "", "timestamp": "now"}}'::jsonb,
true, NOW(), NOW()),
(decode(replace(gen_random_uuid()::text, '-', ''), 'hex'), 'push', 'scheduled task successfully completed', 'backup',
'{"type": "push", "level": 2, "variables": {"task_name": {"type": "string", "required": false}, "duration": {"type": "string", "required": false}, "timestamp": {"type": "string", "required": true}}, "body_template": "Scheduled task{{if .task_name}} {{.task_name}}{{end}} has successfully completed{{if .duration}} in {{.duration}}{{end}} at {{.timestamp}}", "title_template": "Scheduled Task Completed", "default_variables": {"task_name": "", "duration": "", "timestamp": "now"}}'::jsonb,
true, NOW(), NOW()),
(decode(replace(gen_random_uuid()::text, '-', ''), 'hex'), 'push', 'scheduled task failed', 'backup',
'{"type": "push", "level": 4, "variables": {"task_name": {"type": "string", "required": false}, "error_message": {"type": "string", "required": false}, "timestamp": {"type": "string", "required": true}}, "body_template": "Scheduled task{{if .task_name}} {{.task_name}}{{end}} has failed{{if .error_message}}: {{.error_message}}{{end}} at {{.timestamp}}", "title_template": "Scheduled Task Failed", "default_variables": {"task_name": "", "error_message": "", "timestamp": "now"}}'::jsonb,
true, NOW(), NOW()),
(decode(replace(gen_random_uuid()::text, '-', ''), 'hex'), 'push', 'scheduled task partially completed', 'backup',
'{"type": "push", "level": 3, "variables": {"task_name": {"type": "string", "required": false}, "completed_items": {"type": "number", "required": false}, "total_items": {"type": "number", "required": false}, "timestamp": {"type": "string", "required": true}}, "body_template": "Scheduled task{{if .task_name}} {{.task_name}}{{end}} has partially completed{{if .completed_items}}{{if .total_items}} ({{.completed_items}}/{{.total_items}} items){{else}} ({{.completed_items}} items){{end}}{{end}} at {{.timestamp}}", "title_template": "Scheduled Task Partially Completed", "default_variables": {"task_name": "", "completed_items": 0, "total_items": 0, "timestamp": "now"}}'::jsonb,
true, NOW(), NOW()),
(decode(replace(gen_random_uuid()::text, '-', ''), 'hex'), 'push', 'project created', 'account',
'{"type": "push", "level": 2, "variables": {"project_name": {"type": "string", "required": true}, "timestamp": {"type": "string", "required": true}}, "body_template": "A new project ''{{.project_name}}'' has been created at {{.timestamp}}", "title_template": "Project Created", "default_variables": {"project_name": "Project", "timestamp": "now"}}'::jsonb,
true, NOW(), NOW()),
(decode(replace(gen_random_uuid()::text, '-', ''), 'hex'), 'push', 'project deleted', 'account',
'{"type": "push", "level": 2, "variables": {"project_name": {"type": "string", "required": true}, "timestamp": {"type": "string", "required": true}}, "body_template": "Project ''{{.project_name}}'' has been deleted at {{.timestamp}}", "title_template": "Project Deleted", "default_variables": {"project_name": "Project", "timestamp": "now"}}'::jsonb,
true, NOW(), NOW()),
(decode(replace(gen_random_uuid()::text, '-', ''), 'hex'), 'push', 'project member added', 'account',
'{"type": "push", "level": 3, "variables": {"project_name": {"type": "string", "required": false}, "member_email": {"type": "string", "required": false}, "timestamp": {"type": "string", "required": true}}, "body_template": "A new member{{if .member_email}} ({{.member_email}}){{end}} has been added to project{{if .project_name}} {{.project_name}}{{end}} at {{.timestamp}}", "title_template": "Project Member Added", "default_variables": {"project_name": "", "member_email": "", "timestamp": "now"}}'::jsonb,
true, NOW(), NOW()),
(decode(replace(gen_random_uuid()::text, '-', ''), 'hex'), 'push', 'project member deleted', 'account',
'{"type": "push", "level": 3, "variables": {"project_name": {"type": "string", "required": false}, "member_email": {"type": "string", "required": false}, "timestamp": {"type": "string", "required": true}}, "body_template": "Member{{if .member_email}} {{.member_email}}{{end}} has been removed from project{{if .project_name}} {{.project_name}}{{end}} at {{.timestamp}}", "title_template": "Project Member Removed", "default_variables": {"project_name": "", "member_email": "", "timestamp": "now"}}'::jsonb,
true, NOW(), NOW()),
(decode(replace(gen_random_uuid()::text, '-', ''), 'hex'), 'push', 'project invitation accepted', 'account',
'{"type": "push", "level": 2, "variables": {"project_name": {"type": "string", "required": false}, "invitee_email": {"type": "string", "required": false}, "timestamp": {"type": "string", "required": true}}, "body_template": "Invitation to project{{if .project_name}} {{.project_name}}{{end}} has been accepted{{if .invitee_email}} by {{.invitee_email}}{{end}} at {{.timestamp}}", "title_template": "Project Invitation Accepted", "default_variables": {"project_name": "", "invitee_email": "", "timestamp": "now"}}'::jsonb,
true, NOW(), NOW()),
(decode(replace(gen_random_uuid()::text, '-', ''), 'hex'), 'push', 'project invitation declined', 'account',
'{"type": "push", "level": 2, "variables": {"project_name": {"type": "string", "required": false}, "invitee_email": {"type": "string", "required": false}, "timestamp": {"type": "string", "required": true}}, "body_template": "Invitation to project{{if .project_name}} {{.project_name}}{{end}} has been declined{{if .invitee_email}} by {{.invitee_email}}{{end}} at {{.timestamp}}", "title_template": "Project Invitation Declined", "default_variables": {"project_name": "", "invitee_email": "", "timestamp": "now"}}'::jsonb,
true, NOW(), NOW()),
(decode(replace(gen_random_uuid()::text, '-', ''), 'hex'), 'push', 'project name updated', 'account',
'{"type": "push", "level": 2, "variables": {"old_name": {"type": "string", "required": false}, "new_name": {"type": "string", "required": true}, "timestamp": {"type": "string", "required": true}}, "body_template": "Project name has been updated{{if .old_name}} from {{.old_name}}{{end}} to {{.new_name}} at {{.timestamp}}", "title_template": "Project Name Updated", "default_variables": {"old_name": "", "new_name": "Project", "timestamp": "now"}}'::jsonb,
true, NOW(), NOW()),
(decode(replace(gen_random_uuid()::text, '-', ''), 'hex'), 'push', 'project description updated', 'account',
'{"type": "push", "level": 2, "variables": {"project_name": {"type": "string", "required": false}, "timestamp": {"type": "string", "required": true}}, "body_template": "Project description{{if .project_name}} for {{.project_name}}{{end}} has been updated at {{.timestamp}}", "title_template": "Project Description Updated", "default_variables": {"project_name": "", "timestamp": "now"}}'::jsonb,
true, NOW(), NOW()),
(decode(replace(gen_random_uuid()::text, '-', ''), 'hex'), 'push', 'project storage limit updated', 'account',
'{"type": "push", "level": 3, "variables": {"project_name": {"type": "string", "required": false}, "old_limit": {"type": "string", "required": false}, "new_limit": {"type": "string", "required": true}, "timestamp": {"type": "string", "required": true}}, "body_template": "Storage limit for project{{if .project_name}} {{.project_name}}{{end}} has been updated{{if .old_limit}} from {{.old_limit}}{{end}} to {{.new_limit}} at {{.timestamp}}", "title_template": "Storage Limit Updated", "default_variables": {"project_name": "", "old_limit": "", "new_limit": "0 GB", "timestamp": "now"}}'::jsonb,
true, NOW(), NOW()),
(decode(replace(gen_random_uuid()::text, '-', ''), 'hex'), 'push', 'project bandwidth limit updated', 'account',
'{"type": "push", "level": 3, "variables": {"project_name": {"type": "string", "required": false}, "old_limit": {"type": "string", "required": false}, "new_limit": {"type": "string", "required": true}, "timestamp": {"type": "string", "required": true}}, "body_template": "Bandwidth limit for project{{if .project_name}} {{.project_name}}{{end}} has been updated{{if .old_limit}} from {{.old_limit}}{{end}} to {{.new_limit}} at {{.timestamp}}", "title_template": "Bandwidth Limit Updated", "default_variables": {"project_name": "", "old_limit": "", "new_limit": "0 GB", "timestamp": "now"}}'::jsonb,
true, NOW(), NOW()),
(decode(replace(gen_random_uuid()::text, '-', ''), 'hex'), 'push', 'project limit error', 'account',
'{"type": "push", "level": 4, "variables": {"project_name": {"type": "string", "required": false}, "limit_type": {"type": "string", "required": false}, "current_usage": {"type": "string", "required": false}, "limit": {"type": "string", "required": false}, "timestamp": {"type": "string", "required": true}}, "body_template": "Project limit error{{if .project_name}} for {{.project_name}}{{end}}{{if .limit_type}}: {{.limit_type}} limit exceeded{{end}}{{if .current_usage}}{{if .limit}} ({{.current_usage}}/{{.limit}}){{end}}{{end}} at {{.timestamp}}", "title_template": "Project Limit Exceeded", "default_variables": {"project_name": "", "limit_type": "", "current_usage": "", "limit": "", "timestamp": "now"}}'::jsonb,
true, NOW(), NOW()),
...
true, NOW(), NOW());`,
			...
			},
			{
				...
				Version:     383,
				Action: migrate.SQL{
					`CREATE TABLE IF NOT EXISTS mail_export_jobs (
						id text NOT NULL,
					...
					`CREATE INDEX IF NOT EXISTS mail_export_jobs_status_created_at_index ON mail_export_jobs ( status, created_at );`,
					`CREATE INDEX IF NOT EXISTS mail_export_jobs_status_expires_at_index ON mail_export_jobs ( status, expires_at );`,
					`CREATE INDEX IF NOT EXISTS mail_export_jobs_status_started_at_index ON mail_export_jobs ( status, started_at );`,
				},
			},
			{
				DB:          &db.migrationDB,
			...
			{
				DB:          &db.migrationDB,
				Description: "add payment gateway tables and payment_plans.provider_plan_ids",
				Version:     389,
				Action: migrate.SQL{
					`ALTER TABLE payment_plans ADD COLUMN IF NOT EXISTS provider_plan_ids jsonb;`,
					...
					`CREATE INDEX IF NOT EXISTS payment_methods_user_id_provider_index ON payment_methods ( user_id, provider );`,
					...
					`CREATE INDEX IF NOT EXISTS payment_attempts_user_id_index ON payment_attempts ( user_id );`,
					...
					`CREATE INDEX IF NOT EXISTS payment_subscriptions_user_id_provider_index ON payment_subscriptions ( user_id, provider );`,
			...
			},
			{
				...
				Action: migrate.SQL{
					...
					`CREATE INDEX IF NOT EXISTS seller_plans_active_sort_index ON seller_plans ( active, sort_order );`,
					`CREATE TABLE IF NOT EXISTS seller_user_plan_assignments (
						id bytea NOT NULL,
						reseller_id bytea NOT NULL,
						user_id bytea NOT NULL,
					...
					`CREATE INDEX IF NOT EXISTS seller_user_plan_assignments_reseller_id_index ON seller_user_plan_assignments ( reseller_id );`,
					`CREATE INDEX IF NOT EXISTS seller_user_plan_assignments_user_id_status_index ON seller_user_plan_assignments ( user_id, status );`,
					`CREATE INDEX IF NOT EXISTS seller_user_plan_assignments_status_plan_starts_at_index ON seller_user_plan_assignments ( status, plan_starts_at );`,
					`CREATE INDEX IF NOT EXISTS seller_user_plan_assignments_status_plan_ends_at_index ON seller_user_plan_assignments ( status, plan_ends_at );`,
					`CREATE TABLE IF NOT EXISTS seller_invoices (
						id bytea NOT NULL,
						reseller_id bytea NOT NULL,
						period_start timestamp with time zone NOT NULL,
					...
					`CREATE INDEX IF NOT EXISTS seller_invoices_reseller_id_index ON seller_invoices ( reseller_id );`,
					`CREATE TABLE IF NOT EXISTS seller_invoice_lines (
						id bytea NOT NULL,
					...
					`CREATE INDEX IF NOT EXISTS seller_invoice_lines_invoice_id_index ON seller_invoice_lines ( invoice_id );`,
					`CREATE TABLE IF NOT EXISTS seller_billing_notifications (
						id bytea NOT NULL,
						reseller_id bytea NOT NULL,
						user_id bytea,
					...
					`CREATE INDEX IF NOT EXISTS seller_billing_notifications_reseller_id_index ON seller_billing_notifications ( reseller_id );`,
				},
			},
			{
				DB:          &db.migrationDB,
				...
				Version:     391,
				Action: migrate.SQL{
					`ALTER TABLE seller_plans ADD COLUMN IF NOT EXISTS description text NOT NULL DEFAULT '';`,
					`ALTER TABLE seller_plans ADD COLUMN IF NOT EXISTS features jsonb NOT NULL DEFAULT '[]'::jsonb;`,
					`ALTER TABLE seller_plans ADD COLUMN IF NOT EXISTS recommended boolean NOT NULL DEFAULT false;`,
				},
			},
			{
				DB:          &db.migrationDB,
				...
				Version:     392,
				Action: migrate.SQL{
					`ALTER TABLE seller_plans ADD COLUMN IF NOT EXISTS payment_plan_id bigint;`,
				},
			},
			{
				DB:          &db.migrationDB,
				...
				Action: migrate.SQL{
					`DROP INDEX IF EXISTS seller_plans_active_sort_index;`,
					`ALTER TABLE seller_plans DROP COLUMN IF EXISTS sort_order;`,
					`CREATE INDEX IF NOT EXISTS seller_plans_active_name_index ON seller_plans ( active, name );`,
				},
			},
			{
				DB:          &db.migrationDB,
