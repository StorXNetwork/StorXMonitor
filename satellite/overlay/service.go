...
type DB interface {
	...
	GetOnlineNodesForAuditRepair(ctx context.Context, nodeIDs []storxnetwork.NodeID, onlineWindow time.Duration) (map[storxnetwork.NodeID]*NodeReputation, error)
	...
	GetNodes(ctx context.Context, nodeIDs storxnetwork.NodeIDList, onlineWindow, asOfSystemInterval time.Duration) (_ []nodeselection.SelectedNode, err error)
	...
	GetNodesNetwork(ctx context.Context, nodeIDs []storxnetwork.NodeID) (nodeNets []string, err error)
	// GetNodesNetworkInOrder returns the last_net subnet for each storage node in order of the requested nodeIDs.
	GetNodesNetworkInOrder(ctx context.Context, nodeIDs []storxnetwork.NodeID) (nodeNets []string, err error)
	...
	TestNodeCountryCode(ctx context.Context, nodeID storxnetwork.NodeID, countryCode string) (err error)
...
// GetOnlineNodesForAuditRepair returns a map of nodes for the supplied nodeIDs.
func (service *Service) GetOnlineNodesForAuditRepair(ctx context.Context, nodeIDs []storxnetwork.NodeID) (_ map[storxnetwork.NodeID]*NodeReputation, err error) {
	defer mon.Task()(&ctx)(&err)
...
func (service *Service) GetNodes(ctx context.Context, nodeIDs storxnetwork.NodeIDList) (records []nodeselection.SelectedNode, err error) {
	defer mon.Task()(&ctx)(&err)
...
// GetParticipatingNodes returns all known participating nodes (this includes all known nodes
// excluding nodes that have been disqualified or gracefully exited).
func (service *Service) GetParticipatingNodes(ctx context.Context) (records []nodeselection.SelectedNodeWithExtendedData, err error) {
	defer mon.Task()(&ctx)(&err)
...
*/
func (service *Service) UpdateCheckIn(ctx context.Context, node NodeCheckInInfo, timestamp time.Time, config NodeSelectionConfig) (err error) {
	defer mon.Task()(&ctx)(&err)
...
func (service *Service) TestNodeCountryCode(ctx context.Context, nodeID storxnetwork.NodeID, countryCode string) (err error) {
