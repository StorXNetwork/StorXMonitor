...
func cmdSellerRun(cmd *cobra.Command, args []string) (err error) {
	...
	db, err := satellitedb.Open(ctx, log.Named("db"), runCfg.Database, satellitedb.Options{
		ApplicationName:   "satellite-seller",
		APIKeysLRUOptions: runCfg.APIKeysLRUOptions(),
	})
	if err != nil {
		return errs.New("Error starting master database on satellite seller: %+v", err)
	}
	defer func() {
	...
	metabaseDB, err := metabase.Open(ctx, log.Named("metabase"), runCfg.Config.Metainfo.DatabaseURL,
		runCfg.Config.Metainfo.Metabase("satellite-seller"))
	if err != nil {
		return errs.New("Error creating metabase connection on satellite seller: %+v", err)
	}
	defer func() {
		err = errs.Combine(err, metabaseDB.Close())
	}()

	peer, err := satellite.NewSeller(log, identity, db, metabaseDB, &runCfg.Config)
	if err != nil {
		return err
	}

	if err := process.InitMetricsWithCertPath(ctx, log, nil, runCfg.Identity.CertPath); err != nil {
		log.Warn("Failed to initialize telemetry batcher on satellite seller", zap.Error(err))
	}
