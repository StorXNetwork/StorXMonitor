...
pushd "$STORJ_NETWORK_DIR"/release
  case $GOOS in
    linux)    GOOS=linux GOARCH=$(go env GOARCH) go build -tags noquic -o "$RELEASE_BIN"/storagenode -v github.com/StorXNetwork/StorXMonitor/cmd/storagenode   ;;
    *)        CGO_ENABLED=1 GOOS=linux GOARCH=$(go env GOARCH) CC=$STORJ_CC_TARGET CXX=$STORJ_CXX_TARGET go build -tags noquic -o "$RELEASE_BIN"/storagenode -v github.com/StorXNetwork/StorXMonitor/cmd/storagenode ;;
  esac
  GOOS=linux GOARCH=$(go env GOARCH) go build -tags noquic -o "$RELEASE_BIN"/satellite -v github.com/StorXNetwork/StorXMonitor/cmd/satellite
  GOOS=$GOOS GOARCH=$(go env GOARCH) go build -tags noquic -o "$RELEASE_BIN"/uplink -v -ldflags "-X 'github.com/StorXNetwork/uplink.maxSegmentSize=6MiB'" github.com/StorXNetwork/StorXMonitor/cmd/uplink
popd
...
storxnetwork-up local satellite-api,storagenode -d "$RELEASE_BIN"
# persist the 5 nodes that will be restarted with branch binaries
mkdir -p {storagenode1/storxnetwork,storagenode2/storxnetwork,storagenode3/storxnetwork,storagenode4/storxnetwork,storagenode5/storxnetwork}
storxnetwork-up persist storagenode1,storagenode2,storagenode3,storagenode4,storagenode5
...
cd "$SCRIPTDIR"
  case $GOOS in
    linux)    GOBIN=$BRANCH_BIN GOOS=linux GOARCH=$(go env GOARCH) go install -race github.com/StorXNetwork/StorXMonitor/cmd/storagenode 2>&1   ;;
    *)        CGO_ENABLED=1  GOBIN=$BRANCH_BIN GOOS=linux GOARCH=$(go env GOARCH) CC=$STORJ_CC_TARGET CXX=$STORJ_CXX_TARGET go install -race github.com/StorXNetwork/StorXMonitor/cmd/storagenode 2>&1 ;;
  esac
GOBIN=$BRANCH_BIN GOOS=linux GOARCH=$(go env GOARCH) go install -race github.com/StorXNetwork/StorXMonitor/cmd/satellite 2>&1
GOBIN=$BRANCH_BIN GOOS=$GOOS GOARCH=$(go env GOARCH) go install -race -ldflags "-X 'github.com/StorXNetwork/uplink.maxSegmentSize=6MiB'" github.com/StorXNetwork/StorXMonitor/cmd/uplink 2>&1
...
storxnetwork-up local satellite-api,storagenode1,storagenode2,storagenode3,storagenode4,storagenode5 -d $BRANCH_BIN
...
docker compose exec -T satellite-api storxnetwork-up util wait-for-port storagenode1:30001
docker compose exec -T satellite-api storxnetwork-up util wait-for-port storagenode2:30011
docker compose exec -T satellite-api storxnetwork-up util wait-for-port storagenode3:30021
docker compose exec -T satellite-api storxnetwork-up util wait-for-port storagenode4:30031
docker compose exec -T satellite-api storxnetwork-up util wait-for-port storagenode5:30041
