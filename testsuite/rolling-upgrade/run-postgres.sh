...
trap 'docker compose -f ./testsuite/rolling-upgrade/docker-compose-postgres.yaml down' EXIT
...
docker compose -f testsuite/rolling-upgrade/docker-compose-postgres.yaml up \
	--build \
